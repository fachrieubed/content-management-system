            ...
            <div class="row">
                            ...
                            <div class="col-md-6" data-aos="fade">
                                <article class="blog-post">
                                    <div class="post-slider slider-sm rounded">
                                        ...
                                        @if ($article->image != "")
                                        <img loading="lazy" decoding="async" src="{{ asset('storage/'.$article->image) }}" alt="Post Thumbnail" width="350">
                                        @endif


                                    </div>
                                    <div class="pt-4">
                                        <p class="mb-3">{{ \Carbon\Carbon::parse($article->created_at)->locale('id')->translatedFormat('D, d M Y') }}</p>
                                        <h2 class="h4"><a class="text-black" wire:navigate wire:navigate href="{{ route('blogDetail', $article->id) }}">{{ $article->title }}</a></h2>
                                    </div>
                ...
                <div class="col-lg-3">
                    ...
                    <div class="widget">
                        ...
                        @foreach($latestArticles as $latestArticles)
                        <ul class="list-unstyled widget-list">
                            <li class="d-flex widget-post align-items-center">
                                <a class="text-black" href="{{ route('blogDetail', $latestArticles->id) }}">
                                    <div class="widget-post-image flex-shrink-0 me-3">
                                        @if ($latestArticles->image != "")
                                        <img loading="lazy" decoding="async" src="{{ asset('storage/'.$latestArticles->image) }}" alt="Post Thumbnail" width="350">
                                        @endif
                                    </div>
                                </a>
                                <div class="flex-grow-1">
                                    <h5 class="h6 mb-0"><a class="text-black" href="{{ route('blogDetail', $latestArticles->id) }}">{{ $latestArticles->title }}</a></h5>
                                    <small>{{ \Carbon\Carbon::parse($latestArticles->created_at)->locale('id')->translatedFormat('D, d M Y') }}</small>
                                </div>
                            </li>
                        </ul>
                        @endforeach
